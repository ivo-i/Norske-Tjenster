...
const SITEMAP_URL = 'https://kiwi.no/sitemap/content.xml';
const STORE_BASE = 'https://kiwi.no/finn-butikk';
...
async function listStores() {
  ...
  const seen = new Set();
  const out = [];
  const locRe = /<loc>(https:\/\/kiwi\.no\/finn-butikk\/[^<]+)<\/loc>/gi;
  let m;
  while ((m = locRe.exec(raw)) !== null) {
    const url = m[1].replace(/\/$/, '');
    const slug = url.split('/').pop();
    if (!slug || !/^kiwi-/i.test(slug) || seen.has(slug)) continue;
    seen.add(slug);
    out.push({
      slug,
      url: `${url}/`,
