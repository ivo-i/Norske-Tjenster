...
module.exports = class GroceryHoursDevice extends Device {
  ...
  async onInit() {
    ...
    this._openedTrigger = this.homey.flow.getDeviceTriggerCard('grocery_shop_opened');
    this._closedTrigger = this.homey.flow.getDeviceTriggerCard('grocery_shop_closed');
  ...
  async update() {
    ...
    try {
      ...
      if (this._consecutiveErrors >= MAX_STRIKES) {
        await this.setUnavailable(this.homey.__('grocery.device.invalid_chain'));
      }
      this._updating = false;
      return;
    }
    ...
    try {
      ...
      if (!slug) {
        ...
        this._consecutiveErrors += 1;
        if (this._consecutiveErrors >= MAX_STRIKES) {
          await this.setUnavailable(this.homey.__('grocery.device.missing_slug'));
        }
        return;
      }
      const numericId = this.getSetting('grocery_store_numeric_id') || (await this.getStoreValue('groceryNumericId')) || '';
      ...
      if (needsRepair) {
        ...
        this.log('Grocery: attempting store repair (404 or unparsable opening hours)', chainId);
        ...
        const targetSlug =
          newSlug != null && String(newSlug).trim() !== '' ? String(newSlug).trim() : slug;
        if (targetSlug !== slug) {
          await this.setSettings({ store_slug: targetSlug });
        }
        page = await grocery.fetchStorePage(chainId, targetSlug);
        parsed = page.ok && page.html ? chain.parseStoreHtml(page.html) : { jsonLd: null, numericId: null };
        jsonLd = parsed.jsonLd;
      ...
      const settingsPatch = {};
      if (newNumeric && String(newNumeric) !== String(this.getSetting('grocery_store_numeric_id'))) {
        settingsPatch.grocery_store_numeric_id = String(newNumeric);
      }
      ...
      if (newNumeric) {
        await this.setStoreValue('groceryNumericId', String(newNumeric));
      }
      ...
      await this.setCapabilityValue('grocery_shop_open', st.open);
      await this.setCapabilityValue('grocery_shop_status', st.statusText);
      await this.setCapabilityValue('grocery_shop_next_event', st.nextEventText);
      ...
      if (this._suppressEdgeTriggers) {
        ...
        if (st.open) {
          ...
          }, {}).catch((err) => this.error('grocery_shop_opened trigger:', err));
        } else {
          await this._closedTrigger.trigger(this, {
            store_name: name,
            detail: st.nextEventText,
          }, {}).catch((err) => this.error('grocery_shop_closed trigger:', err));
        }
        this._lastOpenState = st.open;
      } else {
        this._lastOpenState = st.open;
      ...
      if (e && e.message === 'STORE_REPAIR_COOLDOWN') {
        this.log('Grocery: store repair cooldown, skipping update');
      } else {
        this.error('Grocery update failed:', e);
        this._consecutiveErrors += 1;
        if (this._consecutiveErrors >= MAX_STRIKES) {
          await this.setUnavailable(this.homey.__('grocery.device.unavailable'));
        }
      }
    } finally {
      this._updating = false;
  ...
  async onSettings({ oldSettings, newSettings, changedKeys }) {
    ...
    if (changedKeys.includes('store_slug')) {
      ...
      try {
        ...
        await this.setStoreValue('groceryNumericId', String(loaded.numericId || ''));
